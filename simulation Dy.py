...
def simulationTime():
    ...
    while(True):
        ...
        if(timeElapsed==simTime):
            ...
            # --- Save dynamic result to CSV ---
            # file placed inside Charts folder next to this file
            result_dir = os.path.join(os.path.dirname(__file__), 'Charts')
            os.makedirs(result_dir, exist_ok=True)
            result_path = os.path.join(result_dir, 'dynamic_results.csv')
            ...
            try:
                with open(result_path, 'a', newline='') as f:
                    writer = csv.writer(f)
                    if write_header:
                        writer.writerow(['No.', 'Dynamic'])
                        attempt_no = 1
                    else:
                        # determine next attempt number robustly
                        with open(result_path, 'r', newline='') as rf:
                            reader = csv.reader(rf)
                            rows = list(reader)
                            if rows and rows[0] and str(rows[0][0]).lower().startswith('no'):
                                attempt_no = len(rows)  # header + N rows -> next No. = len(rows)
                            else:
